    ...
    <div class="modal-content">
        ...
        <form @submit.prevent="emit('addBlockToSchedule')">
            <div class="modal-body">
              <div class="alert alert-danger" v-if="formErrors" role="alert">
  {{formErrors}}
</div>
            <div class="mb-3">
...
<option  :selected="day === formData.startDay" :value="day" v-for="day in days.filter(day=>day!='Saturday' && day !='Sunday')" :key="day">{{day }}</option>
    ...
    <div id="EndDay" class="form-text"> <span v-if="days[formData.duration  -1 + days.indexOf(formData.startDay)]"> End date will be  {{days[formData.duration  -1 + days.indexOf(formData.startDay)]}}</span></div>
  </div>  
         <div class="mb-3">
